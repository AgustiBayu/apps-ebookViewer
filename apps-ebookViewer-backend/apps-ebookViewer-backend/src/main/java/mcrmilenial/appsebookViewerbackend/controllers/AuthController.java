...
import mcrmilenial.appsebookViewerbackend.models.response.MessageResponese;
...
public class AuthController {
    ...
    public ResponseEntity<?> authenticateUser(@RequestBody LoginRequest request) {
        ...
        return ResponseEntity.ok().body(new JwtResponse(token, principal.getUsername(), principal.getEmail(), roles));
    }

    @PostMapping("/signup")
    public ResponseEntity<?> registerUser(@RequestBody SignUpRequest signUpRequest) {
        if (userRepository.existsByUsername(signUpRequest.getUsername())) {
            return ResponseEntity.badRequest().body(new MessageResponese("Error: Username Is Alredy"));
        } else if (userRepository.existsByEmail(signUpRequest.getEmail())) {
            return ResponseEntity.badRequest().body(new MessageResponese("Error: Email Is Alredy"));
        }
        User user = new User(
        ...
        if (userRoles == null) {
            Role mahasiswaRole = roleRepository.findByName(StatusRoles.HASASISWA)
        ...
        return ResponseEntity.ok(new MessageResponese("User registered successfully!"));
    }
}
